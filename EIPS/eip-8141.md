...
Frame transactions can be used by accounts who do not have deployed code, nor an [EIP-7702](./eip-7702.md) delegation indicator, via the "default code" mechanism. Default code provides the canonical ECDSA signature-carrier behavior in-band, so an EOA can act as its own verifier without first deploying code. Conceptually, an empty-code account behaves as if delegated to a canonical ECDSA verifier with the semantics defined below.
