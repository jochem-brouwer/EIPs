...
This EIP removes the storage-clearing refund (`STORAGE_CLEAR_REFUND`, as defined by [EIP-8038](./eip-8038.md)) and the transaction refund cap ([EIP-3529](./eip-3529.md)). One refund remains: [EIP-8038](./eip-8038.md)'s net-metered `STORAGE_WRITE` reversal, which returns gas charged earlier in the same transaction when a storage slot is restored to its transaction-start value. Because that reversal can never exceed the charges it undoes, no cap is needed.

## Motivation

Refunds were introduced to incentivize "good state hygiene": clearing storage that is no longer needed. The incentive has not worked in practice, and state growth is now addressed by pricing state creation directly ([EIP-8037](./eip-8037.md)), not by cleanup rebates.

What refunds still cost the protocol is settlement complexity: a refund counter that must be capped at 20% of gas used ([EIP-3529](./eip-3529.md)), a cap that interacts non-obviously with the calldata floor of [EIP-7623](./eip-7623.md) / [EIP-7976](./eip-7976.md) and with the two gas dimensions of [EIP-8037](./eip-8037.md), and a cross-transaction subsidy — the clearing refund, once abused by GasToken — that every client and gas estimator must model. Most refunds are already gone by the fork this EIP targets: `SELFDESTRUCT` ([EIP-3529](./eip-3529.md)), the [EIP-7702](./eip-7702.md) per-authorization refund ([EIP-2780](./eip-2780.md)), and refunds in block-level gas accounting ([EIP-7778](./eip-7778.md)).

That leaves the `SSTORE` refund rules, which split cleanly in two. The `STORAGE_WRITE` reversal is genuine net metering: a slot that ends the transaction at its starting value pays only access costs. The clearing refund is a cross-transaction incentive with no remaining justification. This EIP removes the latter, keeps the former, and — since the former is bounded by charges made in the same transaction — removes the refund cap as well.
...
The original (2021) version removed all refunds, judging partial retention not worth the complexity. Since then, [EIP-3529](./eip-3529.md), [EIP-7778](./eip-7778.md) and [EIP-2780](./eip-2780.md) removed most refunds, and [EIP-8038](./eip-8038.md) restructured `SSTORE` into explicit access/write components whose net metering *is* the reset refund. In that model the write reversal carries the accounting semantics and the clearing refund carries all the residual complexity, so the line is drawn between them.
