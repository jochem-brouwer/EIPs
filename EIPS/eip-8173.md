---
eip: 8173
title: Static Control Flow for the EVM
...
This document serves as background material for proposals for static control flow, including [EIP-615](./eip-615): Subroutines and Static Jumps for the EVM, [EIP-4750](./eip-4750): EOF Functions, [EIP-7979](./eip-7979): Call and Return Opcodes, [EIP-8013](./eip-8013): Static Relative Jumps, discussions around RISC-V migration and ZK verification infrastructure, and discussions still to come.
...
A _ control flow graph (CFG)_ is a directed graph representation of a program where:
...
Dynamic jumps make dynamic control flow possible, make quadraticaly complex _CFGs_ possible, and can make static analysis of control difficult to impossible.  Dynamic jumps are not a problem for machines which run on physical hardware.  But they are uncommon in virtual machines whose code is the source for downstream tools.  This is because, as we will illustrate below, building and traversing a dynamic control flow graph can take quadratic space and time.  For this reason Java and Wasm do not support dynamic jumps and CLI carefully restricts them.
...
Four EIPs currently specify new EVM opcodes and semantics for static control flow:  [EIP-615: Subroutines and Static Jumps for the EVM](./eip-615), [EIP-4750: EOF - Functions](./eip-4750), [EIP-7979: Call and Return Opcodes for the EVM](./eip-7979), and [EIP-8013: Static relative jumps and calls for the EVM](./eip-8013).  They are all implemented with the standard Turing return stack architecture, and are for the most part compatible with each other other.  In particular, EIP-7979 can be used to implement every other EIP here.
