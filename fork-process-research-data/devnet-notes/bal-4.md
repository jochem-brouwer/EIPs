...
:::info
:mega: bal-devnet-4 target launch: **Wednesday 29.04**.
:::
...
| EIP-8159 empty BAL response | `0xc0` (empty list) | `0x80` (empty string) — [EIPs#11553](https://github.com/ethereum/EIPs/pull/11553) merged 2026-04-21, **after `snøbal-devnet-4@v1.0.0` cut**. Matches EIP-8189 / snap/1 `ByteCodes`/`TrieNodes` convention; avoids collision with empty BALs on chains without system contracts. |
...
| Geth: nested CREATE failure incorrectly restores `GAS_NEW_ACCOUNT` to parent reservoir | Geth | Reported | Tests landed in [execution-specs#2718](https://github.com/ethereum/execution-specs/pull/2718) (rework of closed #2639); now in `snøbal-devnet-4@v1.0.0` |
...
- **Execution Specs:** [`snøbal-devnet-4@v1.0.0`](https://github.com/ethereum/execution-spec-tests/releases/tag/snøbal-devnet-4@v1.0.0) — tagged 2026-04-21 on `devnets/bal/4` (commit `524b446`).
...
### What else is in `tests-bal@v5.7.0`

Full changelog: [compare `tests-bal@v5.6.1...v5.7.0`](https://github.com/ethereum/execution-specs/compare/tests-bal@v5.6.1...tests-bal@v5.7.0).

- **EIP-7708** — `CREATE`/`CREATE2` transfer logs ([EIPs#11474](https://github.com/ethereum/EIPs/pull/11474) merged 2026-04-20); finalization burn-log ordering; coinbase fee emits no log.
- **EIP-7928** — block access index narrowed to `uint32` ([EIPs#11550](https://github.com/ethereum/EIPs/pull/11550) merged 2026-04-20); new BAL coverage for withdrawals, system address, account-only invalid, invalid-path for 2935/4788.
- **EIP-7981** — spec + tests synced to post-ACDT state.
- **EIP-8037 edge coverage** — calldata-floor refund applied to sender, SSTORE refund clamp, nested-frame refunds, cross-EIP 8024/8037 regressions fixed.

### Execution APIs

| PR | Title | Author | Note |
|----|-------|--------|------|
| [#786](https://github.com/ethereum/execution-apis/pull/786) | engine: Restrict no-reorg to the prefix of known finalized | mkalinin | required for glamsterdam-devnets, optional for bal-devnet-4 |
...
articipants:
  ...
  - cl_type: lighthouse
    ...
    cl_log_level: debug
    el_type: erigon
    el_image: ethpandaops/erigon:bal-devnet-4
    el_log_level: debug
    count: 1
  - cl_type: lighthouse
    cl_image: ethpandaops/lighthouse:bal-devnet-4
  ...
  - cl_type: lighthouse
    ...
    cl_log_level: debug
    el_type: nimbus
    el_image: ethpandaops/nimbus-eth1:bal-devnet-4
    el_log_level: debug
    count: 1
network_params:
  preset: minimal
